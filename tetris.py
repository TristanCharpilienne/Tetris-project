...
s_width = 1000 # Largeur de la fenêtre
s_height = 700 # Hauteur de la fenêtre
play_width = 300  # Largeur de la grille de jeu
play_height = 600  # Hauteur de la grille de jeu
block_size = 30 # Taille des blocs
top_left_x = (s_width - play_width) // 2 # Position de la grille de jeu au centre
top_left_y = s_height - play_height - 10# Position de la grille de jeu

# Création des formes (0 correspond à une case remplie), à chaque fois dans toutes les positions possibles de la forme
S = [['.....','.....','..00.','.00..','.....'],
     ['.....','..0..','..00.','...0.','.....']]
...
shapes = [S, Z, I, O, J, L, T ,P] # Tableau des formes
shape_colors = [(255, 60, 60), (255, 150, 60), (255, 255, 50), (160, 255, 128), (0, 180, 255), (166, 120, 255), (140, 220, 255) , (255,150,220)] # Couleurs possibles des formes
...
# On crée la forme sur la map en fonction du mapping des shapes que l'on a défini plus haut (avec les points et les 0)
...
# On vérifie si la position est valide quand on déplace une forme de gauche à droite, ou quand on essaie d'effectuer une rotation sur la forme
...
# On vérifie si les coordonées d'une forme sortent de la map par le haut (y - 1) pour vérifier si le joueur a perdu
...
# On crée une forme en utilisant notre classe Shape
def get_shape():
    global shapes, shape_colors
    # On met l'axe X à 5 pour centrer la position de départ de la forme
    return Shape(5, 0, random.choice(shapes))

# On écrit le texte de fin ("You lost") au milieu de la fenêtre
...
# On dessine le menu de départ
def draw_main_menu(size,color,surface):
    font = pygame.font.SysFont('comicsans', size, bold=False)
    font_title = pygame.font.SysFont('comicsans', size, bold=True)
    
    title = "T E T R I S - T A N"
    text1 = "Appuyez sur Espace pour jouer"
    text2 = "Appuyez sur Échap pour quitter"
    ...
    yt = s_height /2 - label_title.get_height() - 100
...
# On efface une ligne quand elle est remplie
...
def main():
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_DOWN:
                    # Pour accélérer la chute d'une forme
                    current_piece.y += 1
                    if not valid_space(current_piece, grid):
                        current_piece.y -= 1
    ...
    draw_text_middle("Vous avex perdu au jeu de Tristan : " + str(Score[0]), 40, (0, 0, 0), win)
    pygame.display.update()
...
pygame.display.set_caption('T E T R I S - T A N')
